...
int q89() {
	...
	for (int i = 0; i < 1000; ++i) {
		...
		roman_nums.push_back(roman_numeral_helper::convert_to_int<int>(roman_str));
	}
	fin.close();
	...
	for (auto it = roman_nums.begin(); it != roman_nums.end(); ++it) {
		end_char_count += roman_numeral_helper::convert_to_str(*it).size();
	}
...
bool check_all_squares_allowing_6_9_flip(const std::vector<int> &d1, const std::vector<int> &d2) {
	std::vector<bool> d1_contains(10, false), d2_contains(10, false);
	for (auto &f : d1) {
		d1_contains[f] = true;
	}
	for (auto &f : d2) {
	...
	if (!((d1_contains[0] && d2_contains[1]) || (d1_contains[1] && d2_contains[0])) || // 01
		!((d1_contains[0] && d2_contains[4]) || (d1_contains[4] && d2_contains[0])) || // 04
		!((d1_contains[0] && (d2_contains[6] || d2_contains[9])) || ((d1_contains[6] || d1_contains[9]) && d2_contains[0])) || // 09
		!((d1_contains[1] && (d2_contains[6] || d2_contains[9])) || ((d1_contains[6] || d1_contains[9]) && d2_contains[1])) || // 16
		!((d1_contains[2] && d2_contains[5]) || (d1_contains[5] && d2_contains[2])) || // 25
		!((d1_contains[3] && (d2_contains[6] || d2_contains[9])) || ((d1_contains[6] || d1_contains[9]) && d2_contains[3])) || // 36
		!((d1_contains[4] && (d2_contains[6] || d2_contains[9])) || ((d1_contains[6] || d1_contains[9]) && d2_contains[4])) || // 49 and 64
		!((d1_contains[8] && d2_contains[1]) || (d1_contains[1] && d2_contains[8]))) { // 81
...
int q90() {
	...
	std::vector<std::vector<int> > dice = dice_helper::generate_dice(9, 6);
	
	int count = 0;
	for (int i = 0; i < (dice.size() - 1); ++i) {
		for (int j = (i + 1); j < dice.size(); ++j) {
			if (check_all_squares_allowing_6_9_flip(dice[i], dice[j])) {
				++count;
			}
		}
	}
